...
for jj = 1:length(SNR)
    for jjj = 1:cy_num
        ...
        %% 显示发送和接收的数据
        if jjj == 1 % 只在cy_num的第一次循环中显示
            num_to_display = 100; % 显示前100个比特
            disp('发送数据:');
            disp(P_data(1:num_to_display));
            disp('接收数据:');
            disp(rx_c_de(1:num_to_display));
            % 解码接收到的比特数据
            received_str = char(bin2dec(reshape(char(rx_c_de + '0'), 8, []).'))';
            disp('接收到的字符串:');
            disp(received_str);
        end
    end
